import type { AppRouteRequest } from "../api/types.js";
import { AuthorisationError, actorFromHeaders } from "./tenant-auth.js";
import { verifyToken } from "./auth-service.js";
import type { AuthenticatedActor } from "./tenant-auth.js";

export function authenticateRequest(req: AppRouteRequest): AuthenticatedActor {
 ...
 */
export function getRequestUserIdSafe(req: Pick<AppRouteRequest, "headers">): string | null {
 ...
 */
export function getRequestAuthSafe(req: Pick<AppRouteRequest, "headers">): { userId: string; jti?: string } | null {
  const authHeader = req.headers["authorization"];
  if (authHeader) {
    const header = Array.isArray(authHeader) ? authHeader[0] : authHeader;
    const match = header.match(/^Bearer\s+(.+)$/i);
