...
export async function PUT_TEAM(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "team:manage");
...
export async function DEL_TEAM(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "team:manage");
  const teamId = (req.headers["x-resource-id"] as string) ?? "";
...
export async function POST_MEMBER(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "team:manage");
  const teamId = (req.headers["x-resource-id"] as string) ?? "";
...
export async function DEL_MEMBER(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "team:manage");
  const teamId = (req.headers["x-resource-id"] as string) ?? "";
