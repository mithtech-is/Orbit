...
import {
  ...
  queryMileageByRep, queryExpenseKmsAnomalies, queryRouteAdherenceSummary
...
export async function GET_COVERAGE(req: AppRouteRequest, res: AppRouteResponse) {
...
export async function GET_ADHERENCE(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
...
function rangeDays(req: AppRouteRequest, def = 30): number {
...
export async function GET_OFFTARGET(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
...
export async function GET_FUNNEL(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
...
export async function GET_TIME_ON_FIELD(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
  const date = queryParam(req, "date") || new Date().toISOString().slice(0, 10);
...
export async function GET_TRENDS(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
...
export async function GET_VISIT_QUALITY(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
...
export async function GET_FRAUD(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
...
export async function GET_REORDER(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
  const now = Date.now();
...
/** GET /api/v1/reports/mileage/by-rep?days=30 — per-rep mileage + expense summary. */
export async function GET_MILEAGE_BY_REP(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
  const days = rangeDays(req, 30);
  const items = await queryMileageByRep(actor.organisationId, days);
  res.status(200).json({ organisationId: actor.organisationId, dataSource: "mileage_by_rep", days, items });
}

/** GET /api/v1/reports/expense-anomalies?thresholdKm=1 — expenses where manual kms != calculated. */
export async function GET_EXPENSE_ANOMALIES(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
  const threshold = Number(queryParam(req, "thresholdKm")) || 1;
  const items = await queryExpenseKmsAnomalies(actor.organisationId, threshold);
  res.status(200).json({ organisationId: actor.organisationId, dataSource: "expense_anomalies", thresholdKm: threshold, items });
}

/** GET /api/v1/reports/route-adherence-summary?days=30 — per-rep, per-day route adherence. */
export async function GET_ROUTE_ADHERENCE_SUMMARY(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
  const days = rangeDays(req, 30);
  const items = await queryRouteAdherenceSummary(actor.organisationId, days);
  res.status(200).json({ organisationId: actor.organisationId, dataSource: "route_adherence_summary", days, items });
}

/** GET /api/v1/reports/mileage?date=YYYY-MM-DD&ratePerKmCents=50 — per-rep distance + expense. */
export async function GET_MILEAGE(req: AppRouteRequest, res: AppRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "report:read");
  const date = queryParam(req, "date") || new Date().toISOString().slice(0, 10);
