 ...
 * blocks the rep's primary write.
 *
 * Config (env):
...
const ERP_CAPABILITIES = new Set<ErpEntityType>(["customer", "product", "sales_order", "lead", "opportunity", "issue", "payment", "expense_claim", "sales_rep", "employee"]);
...
export function createErpNextProvider(config?: ErpNextConfig): ErpProvider {
  ...
  return {
    ...
    async pushExpenseClaim(payload: ErpExpenseClaimPayload, opts: ErpSyncOptions): Promise<ErpEntityMapping> {
      ...
      // Auto-create Employee on first use — Expense Claim REQUIRES one, and we
      // never want a stray expense to be the thing that fails because HR onboarding
      // wasn't done in ERPNext. (Sales Person already gets pushed for sales-facing
      // roles; Employee is the parallel HR mirror.)
      let employeeMap = await getErpMapping(opts.organisationId, "employee", payload.repUserId);
      if (!employeeMap) {
        const created = await this.pushEmployee(
          { repUserId: payload.repUserId, name: payload.repName, email: payload.repEmail },
          opts
        );
        // Re-read so we keep the same shape as the mapping table row.
        employeeMap = await getErpMapping(opts.organisationId, "employee", payload.repUserId);
        if (!employeeMap) {
          // Shouldn't happen — pushEmployee just saved one — but guard explicitly.
          throw new Error(`failed to ensure ERPNext Employee for rep ${payload.repUserId} (created=${created.erpId})`);
        }
      }
    ...
    /**
     * Push a rep → ERPNext Employee (HR module). The Expense Claim doctype
     * REQUIRES an Employee reference, so this MUST succeed before any expense
     * sync. Idempotent via mapping + hash; uses email to disambiguate same-name
     * reps. company is required on Employee; the configured ERPNEXT_COMPANY is used.
     *
     * NOTE: the ERPNext API user needs the "HR Manager" role (or a custom role
     * with create+write on Employee + Expense Claim). Without it the POST 403s.
     */
    async pushEmployee(payload: ErpEmployeePayload, opts: ErpSyncOptions): Promise<ErpEntityMapping> {
      const hash = hashPayload({ ...payload });
      const existing = await getErpMapping(opts.organisationId, "employee", payload.repUserId);
      if (existing && existing.hash === hash) {
        return toMapping(existing.local_id, existing.erp_id, "push", existing.hash);
      }

      // Split name into first/last for ERPNext (it requires first_name).
      const parts = (payload.name.trim() || payload.email.split("@")[0]).split(/\s+/);
      ...
      const doc: Record<string, unknown> = {
        ...
        employee_name: payload.name.trim() || payload.email,
        company: cfg.company,
        status: "Active",
        // Email is the natural dedup key inside ERPNext Employee.
        personal_email: payload.email,
        prefered_email: payload.email,
      ...
      let erpId: string;
      if (existing) {
        await frappe(cfg, "PUT", `/api/resource/${encodeURIComponent("Employee")}/${encodeURIComponent(existing.erp_id)}`, doc);
        erpId = existing.erp_id;
      } else {
        const created = await frappe<{ data: { name: string } }>(cfg, "POST", `/api/resource/${encodeURIComponent("Employee")}`, doc);
        erpId = created.data.name;
      }
      await saveErpMapping({ organisationId: opts.organisationId, entityType: "employee", localId: payload.repUserId, erpId, hash });
      return toMapping(payload.repUserId, erpId, "push", hash);
    },
    ...
    async pullProducts(opts: ErpSyncOptions): Promise<ErpProductItem[]> {
      const res = await frappe<{ data: Array<{
        name: string; item_code: string; item_name: string;
        stock_uom: string; standard_rate: number | null;
      }> }>(cfg, "GET", "/api/resource/Item?limit_page_length=0&fields=[\"name\",\"item_code\",\"item_name\",\"stock_uom\",\"standard_rate\"]");
      const items = res.data ?? [];
      return items.map((item) => ({
        fieldProductId: item.name,
        sku: item.item_code,
        name: item.item_name,
        uom: item.stock_uom ?? cfg.stockUom,
        unitPriceCents: item.standard_rate != null ? Math.round(item.standard_rate * 100) : 0,
        inventoryAvailable: 0,
        erpId: item.name
      }));
    },

    async searchProducts(query: string, opts: ErpSyncOptions): Promise<ErpProductItem[]> {
      ...
      const res = await frappe<{ data: Array<{
        name: string; item_code: string; item_name: string;
        stock_uom: string; standard_rate: number | null;
      }> }>(cfg, "GET", `/api/resource/Item?filters=${filter}&limit_page_length=50&fields=["name","item_code","item_name","stock_uom","standard_rate"]`);
      const items = res.data ?? [];
      return items.map((item) => ({
        fieldProductId: item.name,
        sku: item.item_code,
    ...
    async pullProductStock(itemCode: string, _opts: ErpSyncOptions): Promise<{ itemCode: string; actualQty: number; reservedQty?: number; pendingQty?: number }> {
      const filter = encodeURIComponent(JSON.stringify([["item_code", "=", itemCode]]));
      const res = await frappe<{ data: Array<{
        item_code: string; actual_qty: number;
        reserved_qty: number; ordered_qty: number;
      }> }>(cfg, "GET", `/api/resource/Bin?filters=${filter}&limit_page_length=1&fields=["item_code","actual_qty","reserved_qty","ordered_qty"]`);
      const bin = (res.data ?? [])[0];
      if (!bin) return { itemCode, actualQty: 0, reservedQty: 0, pendingQty: 0 };
      return {
        itemCode: bin.item_code,
        actualQty: bin.actual_qty ?? 0,
        reservedQty: bin.reserved_qty ?? 0,
        pendingQty: bin.ordered_qty ?? 0
      };
    }
  };
}
