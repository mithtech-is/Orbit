...
export interface MileageRepRow {
  user_id: string;
  user_name: string;
  total_kms: number;
...
/** Per-rep mileage summary — total kms and mileage expense over the last `days`. */
export function queryMileageByRep(organisationId: string, days: number): Promise<MileageRepRow[]> {
  return queryRows<MileageRepRow>(
    `SELECT v.assigned_user_id AS user_id, u.name AS user_name,
            COALESCE(SUM(e.calculated_kms), 0) AS total_kms,
            COALESCE(SUM(e.amount_cents) FILTER (WHERE LOWER(e.category) = 'mileage'), 0) AS total_expense_cents,
            COUNT(DISTINCT v.id) AS visit_count
     FROM visit_expense e
     JOIN visit v ON v.id = e.visit_id AND v.organisation_id = e.organisation_id
     JOIN app_user u ON u.id = v.assigned_user_id AND u.organisation_id = v.organisation_id
     WHERE e.organisation_id = $1 AND v.visit_date >= current_date - make_interval(days => $2)
       AND e.calculated_kms IS NOT NULL
     GROUP BY v.assigned_user_id, u.name
...
export interface ExpenseAnomalyRow {
  ...
  category: string;
  kms: number | null;
  calculated_kms: number | null;
  amount_cents: number;
  rep_name: string;
  outlet_name: string;
}

/** Expenses where manual kms diverges from calculated kms by more than `thresholdKm`. */
export function queryExpenseKmsAnomalies(organisationId: string, thresholdKm = 1.0): Promise<ExpenseAnomalyRow[]> {
  return queryRows<ExpenseAnomalyRow>(
    `SELECT e.id AS expense_id, e.visit_id, e.category, e.kms, e.calculated_kms, e.amount_cents,
            u.name AS rep_name, o.name AS outlet_name
     FROM visit_expense e
     JOIN visit v ON v.id = e.visit_id AND v.organisation_id = e.organisation_id
     JOIN app_user u ON u.id = v.assigned_user_id
     JOIN outlet o ON o.id = v.outlet_id AND o.organisation_id = v.organisation_id
     WHERE e.organisation_id = $1
       AND e.kms IS NOT NULL AND e.calculated_kms IS NOT NULL
       AND ABS(e.kms - e.calculated_kms) > $2
     ORDER BY ABS(e.kms - e.calculated_kms) DESC`,
    [organisationId, thresholdKm]
  );
}

export interface RouteAdherenceSummaryRow {
  user_id: string;
  user_name: string;
  route_date: string;
  planned_stops: number;
  visited_stops: number;
  on_route_stops: number;
  adherence_pct: number;
}

/** Per-rep, per-day route adherence summary. */
export function queryRouteAdherenceSummary(organisationId: string, days: number): Promise<RouteAdherenceSummaryRow[]> {
  return queryRows<RouteAdherenceSummaryRow>(
    `WITH planned AS (
       SELECT rp.assigned_user_id, rp.route_date, COUNT(rs.id) AS stop_count
       FROM route_plan rp
       JOIN route_stop rs ON rs.route_plan_id = rp.id
       WHERE rp.organisation_id = $1 AND rp.route_date >= current_date - make_interval(days => $2)
       GROUP BY rp.assigned_user_id, rp.route_date
     ), visited AS (
       SELECT v.assigned_user_id, v.visit_date, COUNT(DISTINCT v.outlet_id) AS visited_count
       FROM visit v
       WHERE v.organisation_id = $1 AND v.visit_date >= current_date - make_interval(days => $2)
         AND v.checked_in_at IS NOT NULL
       GROUP BY v.assigned_user_id, v.visit_date
     )
     SELECT p.assigned_user_id AS user_id, u.name AS user_name,
            p.route_date::text, p.stop_count AS planned_stops,
            COALESCE(v.visited_count, 0) AS visited_stops,
            0 AS on_route_stops,
            CASE WHEN p.stop_count > 0
              THEN GREATEST(0, ROUND(100.0 * COALESCE(v.visited_count, 0) / p.stop_count))::int
              ELSE 0
            END AS adherence_pct
     FROM planned p
     LEFT JOIN visited v ON v.assigned_user_id = p.assigned_user_id AND v.visit_date = p.route_date
     JOIN app_user u ON u.id = p.assigned_user_id
     ORDER BY p.route_date DESC, adherence_pct ASC`,
    [organisationId, days]
  );
}
