...
export async function readVisitExtras(visitId: string, organisationId: string): Promise<VisitExtras> {
  ...
  const expenses = await queryRows<{ id: string; category: string; amount_cents: number; kms: number | null; note: string | null }>(
    `SELECT id, category, amount_cents, kms, note FROM visit_expense WHERE visit_id = $1 AND organisation_id = $2 ORDER BY created_at`,
    [visitId, organisationId]
  );
  ...
  return {
    ...
    expenses: expenses.map((e) => ({ id: e.id, category: e.category, amountCents: e.amount_cents, kms: e.kms, note: e.note })),
