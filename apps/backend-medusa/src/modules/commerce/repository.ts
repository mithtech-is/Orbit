...
import { bridgeFieldOrderToMedusa, isMedusaBridgeConfigured } from "../../integrations/medusa-client.js";
...
export async function getFieldOrder(organisationId: string, id: string): Promise<FieldOrderRow | undefined> {
  const rows = await queryRows<FieldOrderRow>(
    `SELECT id, organisation_id, outlet_id, rep_user_id, status, source, total_cents, created_at, medusa_order_id
 ...
 * single transaction. The line items are stored in `audit_log.metadata` for now
 * — when Medusa cart/order modules are wired in (Phase 14 follow-up), this
 * function becomes a thin adapter over `cartModuleService.create` + `orderModuleService.create`.
 */
export async function createFieldOrder(input: CreateFieldOrderInput): Promise<CreateFieldOrderResult> {
  ...
  // Captured inside the transaction for the post-commit Medusa bridge call.
  ...
  // Local order persisted. Best-effort mirror to Medusa.
  let medusaOrderId: string | null = null;
  let bridgeError: string | null = null;
  if (isMedusaBridgeConfigured()) {
    try {
      const result = await bridgeFieldOrderToMedusa({
        fieldOrderId: input.id,
      ...
      await pool.query(
        `UPDATE field_order SET medusa_order_id = $1 WHERE id = $2 AND organisation_id = $3`,
        [medusaOrderId, input.id, input.organisationId]
      );
    } catch (err) {
      bridgeError = err instanceof Error ? err.message : "medusa bridge failed";
      console.warn(`[medusa-bridge] field_order ${input.id}: ${bridgeError}`);
    }
  }

  return { id: input.id, totalCents, status: "accepted", medusaOrderId, bridgeError };
}
