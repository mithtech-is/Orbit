...
export async function POST_IMPERSONATE(req: MedusaRouteRequest, res: MedusaRouteResponse) {
  const actor = authenticateRequest(req);
...
export async function POST_RESET_PASSWORD(req: MedusaRouteRequest, res: MedusaRouteResponse) {
  const actor = authenticateRequest(req);
  requireTenantPermission(actor, { organisationId: actor.organisationId }, "user:manage");
 ...
 */
export async function POST_CHANGE_PASSWORD(req: MedusaRouteRequest, res: MedusaRouteResponse) {
  const actor = authenticateRequest(req);
  const body = (req.body as Record<string, unknown>) ?? {};
