...
import { countErpMappings, getErpMapping, deleteErpMapping } from "./erp-mapping-repository.js";
 ...
 */
export async function syncFieldOrderToErp(organisationId: string, input: SyncFieldOrderInput): Promise<void> {
  if (!erpEnabled()) return;
  ...
  try {
    ...
    await getErpProvider().pushSalesOrder(
      {
...
export async function backfillErp(organisationId: string): Promise<{ outlets: number; products: number; leads: number; reps: number }> {
  ...
  // Sales-facing users -> Sales Persons (active reps + managers only).
  ...
  for (const r of reps) await pushSalesRep(organisationId, r);
